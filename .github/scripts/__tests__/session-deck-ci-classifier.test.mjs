...
for (const [name, output, message] of [
  ...
  test(`readChanges rejects ${name}`, async () => {
    await assert.rejects(
      readChanges(SHA_A, SHA_B, async () => output),
      message,
    );
  });
}
...
test('Option 2 path rules preserve known-safe scopes', async () => {
  ...
  for (const repositoryPath of safePaths) {
    assert.deepEqual(
      await classifyPaths([change(repositoryPath)]),
      {
        run_desktop: false,
    ...
    assert.equal(
      (await classifyPaths([change(repositoryPath, 'D')])).run_desktop,
      false,
      repositoryPath,
    );
  }
...
test('empty and mixed diffs OR all path results', async () => {
  ...
  assert.equal(
    (await classifyPaths([change('README.md'), change('apps/session-deck-desktop/web/app.js')]))
      .run_desktop,
    true,
  );
});
    ...
    assert.equal(
      (
        await classifyPaths([
          {
            status,
            paths: ['packages/pi-session-deck/extensions/session-deck/index.ts', 'README.md'],
          },
        ])
      ).run_desktop,
      true,
...
test('the exact PR #150 six-path fixture skips desktop work', async () => {
  ...
  assert.deepEqual(
    await classifyPaths(changes, { readBlob: blobReader(oldManifest, newManifest) }),
    {
      run_desktop: false,
      classification_error: false,
    },
...
for (const [manifestPath, before, after] of cosmeticCases) {
  test(`${manifestPath} allows only same-path cosmetic modifications`, async () => {
    assert.equal(
      (await classifyPaths([change(manifestPath)], { readBlob: blobReader(before, after) }))
        .run_desktop,
      false,
    );
  });
...
test('malformed, non-object, unreadable, and invalid UTF-8 manifests are semantic errors', async (t) => {
  ...
  for (const [name, reader, message] of [
    ['malformed', async (_path, side) => (side === 'old' ? '{' : '{}'), /old .* not valid JSON/],
...
test('local workspace, link, and file dependencies own their target package paths', async () => {
  ...
  assert.deepEqual([...assertLocalDependencyOwnership(manifests)].sort(), [
    'packages/filed',
    'packages/linked',
    'packages/workspace',
  ]);
