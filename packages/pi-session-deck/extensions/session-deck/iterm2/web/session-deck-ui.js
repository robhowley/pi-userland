(function () {
  function mountSessionDeckUI(options = {}) {
    const host = options.host;
    if (!host) {
      throw new Error('Session Deck host is required.');
    }

    const document = options.document ?? globalThis.document;
    const window = options.window ?? globalThis.window ?? globalThis;
    const HTMLButtonElement = window.HTMLButtonElement ?? globalThis.HTMLButtonElement;
    const HTMLInputElement = window.HTMLInputElement ?? globalThis.HTMLInputElement;
    ...
    function formatLaunchContextPreviewSummary(preview) {
      if (
      ...
      if (typeof preview === 'object' && preview !== null && preview.status === 'failed') {
    ...
    const DEFAULT_DOCTOR_COMMAND = '/session-deck iterm2 doctor';
    const doctorCommand = host.doctorCommand ?? DEFAULT_DOCTOR_COMMAND;
    ...
    function isWorktreeLaunchSubmitReady(formState, launchPreview) {
      return (
        ...
        isResolvedWorktreeLaunchPreview(launchPreview)
      );
    }
    ...
    function postWorktreeBasePreview(repoGroup) {
      return host.previewWorktreeBaseRef({
        repoIntent: buildWorktreeRepoIntent(repoGroup),
      });
    }

    function postWorktreeLaunchPreview(formState) {
      return host.previewWorktreeLaunchContext({
        launch: {
          mode: 'tmux-detached',
          agentDir: getWorktreeLaunchAgentDirIntent(formState),
        },
    ...
    function postCreateWorktreeAction(request) {
      return host.createWorktree(request);
    }

    function postCreateSessionAction(request) {
      return host.createSession(request);
    }

    function postOpenTerminalAction(runtimeId) {
      return host.openTerminal(runtimeId);
    }

    function postKillSessionAction(runtimeId) {
      return host.killSession(runtimeId);
    }
    ...
    function summarizeCreateSessionActionResult(repoKey, request, result) {
      ...
      const reused =
        result.status === 'reused-existing' || result.launch?.status === 'reused-existing';
      return {
        sessionKind: 'cwd',
        kind: 'success',
    ...
    function getPreflightFailureMessage(preflight) {
      switch (preflight?.reason) {
        case 'tmux-unavailable':
          return `New Pi session requires tmux on PATH; no worktree was created. Run ${doctorCommand} or install tmux.`;
        case 'pi-command-unavailable':
          return `New Pi session requires the pi executable on PATH; no worktree was created. Run ${doctorCommand} or install Pi.`;
        default:
          return isNonEmptyString(preflight?.message)
            ? preflight.message
            : `New Pi session prerequisites are unavailable; no worktree was created. Run ${doctorCommand}.`;
      }
    }
    ...
    function getCreateSessionPreflightFailureMessage(preflight) {
      switch (preflight?.reason) {
        case 'tmux-unavailable':
          return `New Pi session requires tmux on PATH; no session was launched. Run ${doctorCommand} or install tmux.`;
        case 'pi-command-unavailable':
          return `New Pi session requires the pi executable on PATH; no session was launched. Run ${doctorCommand} or install Pi.`;
        default:
          return isNonEmptyString(preflight?.message)
            ? preflight.message
            : `New Pi session prerequisites are unavailable; no session was launched. Run ${doctorCommand}.`;
      }
    }
    ...
    function getPartialLaunchFailureMessage(launch) {
      switch (launch?.reason) {
        case 'tmux-unavailable':
          return `Worktree kept. Pi did not start because tmux is not available. Run ${doctorCommand} or install tmux, then retry.`;
        case 'pi-command-unavailable':
          return `Worktree kept. Pi did not start because the pi executable is not available. Run ${doctorCommand} or install Pi, then retry.`;
    ...
    function getOpenTerminalActionForRecord(record) {
      return state.openTerminalAction?.runtimeId === record.runtimeId
        ? state.openTerminalAction
        : null;
    }
    ...
    function getOpenTerminalActionFailureMessage(result) {
      return isNonEmptyString(result?.message)
        ? result.message
        : DEFAULT_OPEN_TERMINAL_FAILURE_MESSAGE;
    }
    ...
    function getKillSessionActionForRecord(record) {
      return state.killSessionAction?.runtimeId === record.runtimeId
        ? state.killSessionAction
        : null;
    }
    ...
    function copyTextToClipboard(text) {
      void Promise.resolve(host.copyText(text)).catch(() => {});
    }
    ...
    function createDetailValue(text, linkHref, middleTruncateTail, valueTitle) {
      ...
      if (linkHref) {
        ...
        value.addEventListener('click', (event) => {
          event.preventDefault();
          void Promise.resolve(host.openExternal(linkHref)).catch(() => {});
        });
      }
  ...
  target.SessionDeckUI = { mount: mountSessionDeckUI };
})();
