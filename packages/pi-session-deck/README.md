...
- **Controlled stops.** Request **Stop Pi** for the selected runtime without deleting history, killing tmux, or closing iTerm windows.
...
When the selected runtime should stop, use `k` in the Pi TUI or **Stop Pi** in expanded Toolbelt details. Stop asks for confirmation, verifies Session Deck presence PID/start-time metadata immediately before signaling, then sends `SIGTERM` to the verified Pi process only. It preserves `.jsonl` history and does not kill process groups, send `SIGKILL`, kill tmux, or close iTerm windows.
...
Enable the iTerm2 Python API if prompted, fully quit and reopen iTerm2, then open **Toolbelt → Session Deck**. If the view is missing, stale, or unable to launch/open/stop an agent, run `/session-deck iterm2 doctor`.

`pi-session-deck` installs an iTerm2 Toolbelt view backed by the same public `SessionDeckSnapshot` / `SessionDeckRecord` data that `/session-deck` already uses. Session rows include **↗ Open**, expanded session details include **Stop Pi**, and repo groups include **＋ New**.
...
- The installed `session_deck.py` AutoLaunch script starts one Session Deck iTerm2 process that binds to `127.0.0.1`, reads snapshots through the package-owned helper, runs Open, Stop Pi, and create-worktree actions through dedicated helpers, and exposes the local Unix socket used by `/session-deck` TUI and Toolbelt Open terminal focus.
...
- Local repo builds need `pnpm --dir packages/pi-session-deck run build` before install so the snapshot, Open, Stop Pi, and create-worktree helpers exist in `dist/`.
...
| `k`       | Confirm and stop the selected Pi process with `SIGTERM` only.             |
| `r`       | Refresh.                                                                  |
| `q`       | Close Session Deck.                                                       |
| `esc`     | Cancel the branch/stop prompt, or close Session Deck when no prompt opens. |
...
- Public JSON and the Toolbelt view omit raw terminal metadata, tmux socket paths, pane ids, reveal URLs, and derived attach commands; Toolbelt Open and Stop Pi send only `{ runtimeId }`.
