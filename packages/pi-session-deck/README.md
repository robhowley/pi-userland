...
Session Deck gives each Pi agent a live operating surface across the TUI and iTerm2 Toolbelt. Use it to inspect liveness, branch context, PR state, status chips, and attached terminal targets, then jump back into the right agent without hunting through terminal tabs.
