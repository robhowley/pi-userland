...
const AUTO_DETECT: Array<{ parserId: string; detect: (argv: string[]) => boolean }> = [
  ...
  },
  {
    ...
    detect: (argv) =>
      argv.some(
        (a) => a.startsWith('--junitxml') || a.startsWith('--junit-xml') || a === '--format=junit',
      ),
  },
  {
    parserId: 'cargo-build',
    detect: (argv) =>
      argv.includes('cargo') && argv.includes('build') && hasFlag(argv, '--message-format', 'json'),
  },
  {
    parserId: 'cargo-test',
  ...
  },
  {
    ...
    detect: (argv) =>
      argv.includes('unittest') ||
      (argv.includes('python3') && argv.includes('-m') && argv.includes('unittest')),
  },
  {
