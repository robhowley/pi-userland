...
function createCtx(opts: { contextTokens?: number | null; hasUI?: boolean } = {}): ExtensionContext & ExtensionCommandContext {
...
async function setupWithSession(opts: { contextTokens?: number | null; branchCosts?: number[] } = {}) {
...
describe("session-hygiene", () => {
  ...
  describe("helper functions", () => {
    ...
    it("validates threshold objects", () => {
      ...
      expect(isValidThresholds({ yellow: { cost: -1, context: 60_000 }, red: { cost: 8, context: 120_000 } })).toBe(false);
      expect(isValidThresholds({ yellow: { cost: 2, context: 120_000 }, red: { cost: 8, context: 120_000 } })).toBe(false);
      expect(isValidThresholds({ yellow: { cost: 8, context: 60_000 }, red: { cost: 8, context: 120_000 } })).toBe(false);
      expect(isValidThresholds({ yellow: { cost: 2 }, red: { cost: 8, context: 120_000 } })).toBe(false);
    ...
    it("loads defaults when config is missing, invalid, or unreadable", () => {
      ...
      vi.mocked(fs.readFileSync).mockReturnValue(JSON.stringify({ yellow: { cost: 8, context: 60_000 }, red: { cost: 8, context: 120_000 } }));
      expect(loadConfig()).toEqual(DEFAULT_THRESHOLDS);
    ...
    it("loads valid config from disk", () => {
      ...
      expect(loadConfig()).toEqual({ yellow: { cost: 10, context: 100_000 }, red: { cost: 20, context: 200_000 } });
    });
    ...
    it("saves config to the expected path", () => {
      ...
      expect(fs.writeFileSync).toHaveBeenCalledWith(CONFIG_FILE, JSON.stringify(DEFAULT_THRESHOLDS, null, 2), { mode: 0o600 });
    });
  ...
  describe("/session-hygiene command", () => {
    it("shows current thresholds and allows cancellation", async () => {
      ...
      expect(ctx.ui.notify).toHaveBeenCalledWith(expect.stringContaining("Current Thresholds"), "info");
    ...
    it("saves a preset and refreshes status", async () => {
      ...
      expect(fs.writeFileSync).toHaveBeenCalledWith(CONFIG_FILE, expect.stringContaining('"cost": 10'), { mode: 0o600 });
      expect(ctx.ui.notify).toHaveBeenCalledWith(expect.stringContaining("Configuration saved"), "info");
      expect(vi.mocked(ctx.ui.setStatus).mock.calls.at(-1)).toEqual([
        "session-hygiene",
        "🟢 session healthy",
      ]);
    ...
    it("saves valid custom thresholds", async () => {
      ...
      expect(fs.writeFileSync).toHaveBeenCalledWith(CONFIG_FILE, expect.stringContaining('"cost": 3'), { mode: 0o600 });
    });
    ...
    it("rejects custom thresholds with invalid numbers", async () => {
      ...
      expect(ctx.ui.notify).toHaveBeenCalledWith(expect.stringContaining("Invalid thresholds"), "error");
    ...
    it("keeps the session update even if saving config fails", async () => {
      ...
      expect(ctx.ui.notify).toHaveBeenCalledWith(expect.stringContaining("Applied in this session but could not save to disk"), "warning");
      expect(vi.mocked(ctx.ui.setStatus).mock.calls.at(-1)).toEqual([
        "session-hygiene",
        "🟡 session growing",
      ]);
  ...
  describe("session_compact", () => {
    it("resets running totals and cache stats after compaction", async () => {
      ...
      expect(vi.mocked(ctx.ui.setStatus).mock.calls.at(-1)?.[1]).toBe("🟢 session healthy · 80% cache");
